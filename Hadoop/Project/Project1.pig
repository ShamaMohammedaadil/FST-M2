-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/shama/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/shama/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/shama/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/shama/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/shama/outputs' USING PigStorage('\t');